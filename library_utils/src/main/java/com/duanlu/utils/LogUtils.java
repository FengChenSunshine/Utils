 ...
 * @name LogUtils
 * @author 段露
 * @createDate 2019/07/01 16:22
 * @updateDate 2019/07/01 16:22
 * @version V1.0.0
 * @describe 日志工具栏.
 * 参考：https://www.jianshu.com/p/b806d014162c.
 ********************************/
public class LogUtils {
    ...
    private static String sFolder;//log存储目录
    private static boolean sLogEnabled = true; // log总开关
    private static String sGlobalTag = null; // log标签
    private static boolean sTagIsSpace = true; // log标签是否为空白
    private static boolean sFileEnabled = false;// log写入文件开关
    private static boolean isBorder = true; // log边框开关
    private static int sFilter = V;    // log过滤器
    ...
    public static void init(boolean logcatEnabled, boolean fileEnabled, @Filter int filter, String folder, String globalTag, boolean isBorder) {
        LogUtils.sLogEnabled = logcatEnabled;

        LogUtils.sFileEnabled = fileEnabled;

        LogUtils.sFilter = filter;

        LogUtils.sFolder = folder;

        if (!isSpace(globalTag)) {
            LogUtils.sGlobalTag = globalTag;
            sTagIsSpace = false;
        } else {
            LogUtils.sGlobalTag = "";
            sTagIsSpace = true;
    ...
    private static void printLog(int type, String tag, String msg) {
        if (isBorder) printBorder(type, tag, true);
        int len = msg.length();
    ...
    private synchronized static void print2File(final String tag, final String msg) {
        ...
        final String fullPath = sFolder + date + ".txt";
        ...
        StringBuilder sb = new StringBuilder();
        if (isBorder) sb.append(TOP_BORDER).append(LINE_SEPARATOR);
        sb.append(time)
                .append(tag)
                .append(": ")
                .append(msg)
                .append(LINE_SEPARATOR);
        if (isBorder) sb.append(BOTTOM_BORDER).append(LINE_SEPARATOR);
